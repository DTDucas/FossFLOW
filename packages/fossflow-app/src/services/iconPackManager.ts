...
const PACK_METADATA: Record<IconPackName, string> = {
  ...
  'aws-category': 'AWS Category Icons',
  'aws-core': 'AWS Core Icons',
  'aws-asset': 'AWS Asset Icons',
  'azure-public': 'Azure Public Icons'
};
...
export const loadIconPack = async (packName: IconPackName): Promise<any> => {
  if (
    ['aws-category', 'aws-core', 'aws-asset', 'azure-public'].includes(packName)
  ) {
    const response = await fetch(`/icon-packs/${packName}/manifest.json`);
    if (!response.ok) {
      throw new Error(`Failed to load manifest for ${packName}`);
    }
    const manifest = await response.json();
    return {
      ...manifest,
      icons: manifest.icons.map((icon: any) => ({
        ...icon,
        url: `/icon-packs/${packName}/${icon.url}`
      }))
    };
  }
  ...
  const loadPack = useCallback(
    async (packName: IconPackName) => {
      ...
      try {
        ...
        setLoadedIcons((prev: any) => [...prev, ...flattenedIcons]);
  ...
  const togglePack = useCallback(
    async (packName: IconPackName, enabled: boolean) => {
      if (enabled) {
        ...
        const newEnabledPacks = enabledPacks.filter(
          (p: IconPackName) => p !== packName
        );
        setEnabledPacks(newEnabledPacks);
        saveEnabledPacks(newEnabledPacks);
